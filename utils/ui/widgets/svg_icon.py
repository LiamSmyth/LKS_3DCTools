"""SvgIcon / SvgIconButton / get_icon — replaced by lks_utils QSquareIconButton and button_presets.

This module retains backward-compatible factory functions that use
lks_utils primitives internally.
"""
...
from lks_utils.gui_qt.widgets.button_presets import preset_icon_color
...
def _resolve_svg_path(name: str) -> Path:
    ...
    return pkg_root / "utils" / "ui" / "data" / f"{name}.svg"
...
class SvgIcon(QWidget):
    ...
    lks_utils QSquareIconButton uses a different approach (theme-token
...
class SvgIconButton(QPushButton):
    ...
    Uses lks_utils QSquareIconButton internally for the fixed-size square
