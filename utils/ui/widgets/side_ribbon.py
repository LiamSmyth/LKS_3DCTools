"""SideRibbon — backed by lks_utils QCollapsiblePanel.

This module provides backward-compatible ``SideRibbon`` and ``Side``
classes that wrap the lks_utils ``QCollapsiblePanel`` internally.
...
- Side enum still works; mapped to lks_utils string directions
...
if HAS_QT:
    ...
    class SideRibbon(QCollapsiblePanel):
        """A collapsible side ribbon backed by lks_utils QCollapsiblePanel.
