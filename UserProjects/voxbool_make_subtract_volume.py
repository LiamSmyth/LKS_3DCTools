...
print("Creating Subtract volume for: ", active_element.name())
...
def create_union_volume(el: coat.SceneElement):
    ...
    cloned_element.rename(el.name() + "_Subtract")
    coat.io.step(4)
    cloned_element.selectOne()
    coat.io.step(4)
    ...
    vo.assignLiveBooleans(1)

    print(f"Created Subtract volume: {cloned_element.name()}")
# Create the intersection volume

create_union_volume(active_element)

print("Subtract volume creation complete")