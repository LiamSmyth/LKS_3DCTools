"""
Switch Voxel 4x Polys

This script converts a surface object to voxels with 4x the original polycount,
or converts voxels back to surface mode.
"""
import coat


def switch_vox_4x_polys():
    """Switch between surface and voxel modes with 4x polycount"""
    ...
    target_polycount = current_polycount * 4

    print(
        f"Converting to voxels with 4x polycount: {current_polycount:,} -> {target_polycount:,}")
    ...
    if current_polycount > 0:
        ...
        coat.ui.showInfoMessage(
            f"Converted to Voxels (4x): {new_polycount:,} polys", 3000)
