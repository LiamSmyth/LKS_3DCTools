...
def adjust_polycount_to_uniform_size(obj: coat.SceneElement, ref_obj: coat.SceneElement):
    ...
    if obj.isSculptObject():
        ...
        elif polycount_ratio > 2:
            # If the polycount ratio is greater than 2, we can subdivide the object
            # This is a ballbark, stating that less than 2x polycount isn't worth subdividing

            # Each subdivision quadruples the polycount,
            # so we need to subdivide the object log2(polycount_ratio) times

            # We should cap the subdivisions at 5 times
            subdivide_iterations = min(
                math.ceil(math.log2(resample_ratio)), 5)

            # Perform at least one subdivision
            subdivide_iterations = max(subdivide_iterations, 1)

            for i in range(subdivide_iterations):
                coat.ui.cmd("$VoxTreeBranch.IncRes_HINT.Root")

            print(
                f"Subdivided \" {obj.name()} \" {subdivide_iterations} times, from {cur_polycount} polys to {tgt_polycount} polys, a polycount ratio of {polycount_ratio} \n")

            return False

        # # If the ratio is greater than we need to resample to increase polycount instead.
        else:
            print(
