...
| `$COMBOBOX_QuadQualityqDraft` | Select draft quality | cmd | - |
| `$COMBOBOX_QuadQualityqIntermediate` | Select intermediate quality | cmd | - |
| `$COMBOBOX_QuadQualityqBestQuality` | Select best quality | cmd | - |

**Utility Location:** `_utils/autopo_utils.py`

**Notes:**
- Voxelize polycount field is hidden until voxelize checkbox is enabled - must wait 2 frames with `coat.io.step(2)` before setting
- Quality dropdown: Use `coat.ui.cmd()` with quality-specific strings (NOT `setOption()`)
- Most checkboxes use `setBoolValue(bool)`
- Spinboxes accept `setEditBoxValue(int)` or `setSliderValue(float)`
